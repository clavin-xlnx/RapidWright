...
public class PFRouterNodeBased {
	...
	public void route(){
		PFRouter<Node> router = new PFRouter<Node>(this.design, this.queue, this.rnodesTouched, this.rnodesCreated);
		//sorted nets and connections
		List<Connection<Node>> sortedListOfConnection = new ArrayList<>();
...
//		Collections.sort(sortedListOfConnection, Comparators.FanoutBBConnection);
		...
		List<Netplus<Node>> sortedListOfNetplus = new ArrayList<>();		
...
//		Collections.sort(sortedListOfNetplus, Comparators.FanoutNet);
		...
		ChildRNodeGeneration expan = new ChildRNodeGeneration(this.rnodesCreated, ExpanGranularityOpt.NODE);
		...
		while(router.getItry() < this.nrOfTrials){
			...
			for(RNode<Node> rnode:sortedListOfConnection.get(0).rNodes){
				this.printInfo(rnode.toString());
			}*/
			
			for(Connection<Node> con:sortedListOfConnection){
				if(router.getItry() == 1){
	...
	public void routeACon(PFRouter<Node> router, ChildRNodeGeneration expan, Connection<Node> con){
		router.ripup(con);
	...
	public void outOfTrialIterations(PFRouter<Node> router){
